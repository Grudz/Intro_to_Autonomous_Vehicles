...
#include <ros/ros.h>
#include <tf2_geometry_msgs/tf2_geometry_msgs.h>  // Don't think it's required
...
// Message headers
#include <sensor_msgs/PointCloud2.h>  // input/Ouput cloud message
...
namespace ece6460_pointcloud_example
{
  ...
  class ClusterExample
  {
    ...
    private:
      ...
      void filterRawCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_in,  // Intermediate refactor function, not message callback or anything
                          pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_out);

      ros::Publisher pub_filtered_cloud_;  //Straight point cloud
      ros::Publisher pub_normals_;  // Normal vectors
      ros::Publisher pub_bboxes_;  // Boxes
      ros::Subscriber sub_cloud_;  // Raw cloud
      ...
      pcl::search::Search<pcl::PointXYZ>::Ptr kd_tree_;  // Searchest for nearest nieghbors in PC. reordersn points so more efficent to search
  };
